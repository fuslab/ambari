...
class ZeppelinServiceAdvisor(service_advisor.ServiceAdvisor):
  ...
  def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
    ...
    recommender.recommendZeppelinConfigurationsFromANYSCALE10(configurations, clusterData, services, hosts)
...
class ZeppelinRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendZeppelinConfigurationsFromANYSCALE10(self, configurations, clusterData, services, hosts):
    """
    :type configurations dict
    :type clusterData dict
    :type services dict
        ...
        if "ZEPPELIN_INTP_CLASSPATH_OVERRIDES" in line:
          if sac_enabled:
            if line.lstrip().startswith("#"):
              line = "export ZEPPELIN_INTP_CLASSPATH_OVERRIDES=\"{{external_dependency_conf}}:/opt/anyscale/current/spark-atlas-connector/*\""
            elif "{{external_dependency_conf}}" in line:
              line = line.replace("{{external_dependency_conf}}", "{{external_dependency_conf}}:/opt/anyscale/current/spark-atlas-connector/*")
            else:
              k = line.rfind("\"")
              line = line[:k] + ":/opt/anyscale/current/spark-atlas-connector/*\"" + line[k+1:]
          else:
            if ":/opt/anyscale/current/spark-atlas-connector" in line:
              line = line.replace(":/opt/anyscale/current/spark-atlas-connector/*", "")
            elif "/opt/anyscale/current/spark-atlas-connector" in line:
              line = line.replace("/opt/anyscale/current/spark-atlas-connector/*", "")
