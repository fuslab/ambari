...
class YARNServiceAdvisor(service_advisor.ServiceAdvisor):
  ...
  def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
    ...
    recommender.recommendYARNConfigurationsFromANYSCALE10(configurations, clusterData, services, hosts)
...
class YARNRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendYARNConfigurationsFromANYSCALE10(self, configurations, clusterData, services, hosts):
    putYarnSiteProperty = self.putProperty(configurations, "yarn-site", services)
...
class MAPREDUCE2Recommender(YARNRecommender):
  ...
  def recommendMapReduce2ConfigurationsFromHDP22(self, configurations, clusterData, services, hosts):
    ...
    putMapredProperty('yarn.app.mapreduce.am.command-opts', "-Xmx" + str(int(0.8 * int(configurations["mapred-site"]["properties"]["yarn.app.mapreduce.am.resource.mb"]))) + "m" + " -Danyscale.version=${anyscale.version}")
    servicesList = [service["StackServices"]["service_name"] for service in services["services"]]
