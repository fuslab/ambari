...
STACKS_DIR = os.path.join(SCRIPT_DIR, '../../../1.0/services/RANGER/')
...
class ANYSCALE11RANGERServiceAdvisor(service_advisor.RangerServiceAdvisor):

  def __init__(self, *args, **kwargs):
    self.as_super = super(ANYSCALE11RANGERServiceAdvisor, self)
    self.as_super.__init__(*args, **kwargs)

  def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
    super(ANYSCALE11RANGERServiceAdvisor, self).getServiceConfigurationRecommendations(configurations, clusterData, services, hosts)

    recommender = ANYSCALE11RangerRecommender()
    recommender.recommendRangerConfigurationsFromANYSCALE11(configurations, clusterData, services, hosts)

class ANYSCALE11RangerRecommender(service_advisor.ServiceAdvisor):
  """
  Ranger Recommender suggests properties when adding the service for the first time or modifying configs via the UI.
  """

  def __init__(self, *args, **kwargs):
    self.as_super = super(ANYSCALE11RangerRecommender, self)
    self.as_super.__init__(*args, **kwargs)
    self.initialize_logger("ANYSCALE11RangerRecommender")

  def recommendRangerConfigurationsFromANYSCALE11(self, configurations, clusterData, services, hosts):
