...
STACKS_DIR = os.path.join(SCRIPT_DIR, '../../../1.0/services/HDFS/')
...
class ANYSCALE11HDFSServiceAdvisor(service_advisor.HDFSServiceAdvisor):

  def __init__(self, *args, **kwargs):
    self.as_super = super(ANYSCALE11HDFSServiceAdvisor, self)
    self.as_super.__init__(*args, **kwargs)

    self.initialize_logger("ANYSCALE11HDFSServiceAdvisor")
  ...
  def getServiceConfigurationRecommendations(self, configurations, clusterData, services, hosts):
    super(ANYSCALE11HDFSServiceAdvisor, self).getServiceConfigurationRecommendations(configurations, clusterData, services, hosts)

    recommender = ANYSCALE11HDFSRecommender()
    recommender.recommendHDFSConfigurationsFromANYSCALE11(configurations, clusterData, services, hosts)

class ANYSCALE11HDFSRecommender(service_advisor.ServiceAdvisor):
  """
  HDFS Recommender suggests properties when adding the service for the first time or modifying configs via the UI.
  """

  def __init__(self, *args, **kwargs):
    self.as_super = super(ANYSCALE11HDFSRecommender, self)
    self.as_super.__init__(*args, **kwargs)

  def recommendHDFSConfigurationsFromANYSCALE11(self, configurations, clusterData, services, hosts):
